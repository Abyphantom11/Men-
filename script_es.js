...
class MenuViewer {
    constructor() {
        ...
        this.translateX = 0;
        this.translateY = 0;
        this.menuFolder = 'menu'; // Carpeta con imágenes en español
    ...
    setupNavigation() {
        ...
        document.addEventListener('touchstart', (e) => {
            // BLOQUEAR si hay zoom activo desde el inicio
            if (this.scale > 1) {
                isDragging = false;
                return;
            }
        ...
        document.addEventListener('touchmove', (e) => {
            ...
            // BLOQUEAR si detecta zoom durante el movimiento
            if (this.scale > 1) {
                isDragging = false;
                return;
            }
            
            // No prevenir el scroll vertical
            const currentY = e.touches[0].clientY;
            const deltaY = Math.abs(currentY - startY);
            const deltaX = Math.abs(e.touches[0].clientX - startX);
            
            // Solo prevenir si el movimiento es más horizontal que vertical
            if (deltaX > deltaY) {
                e.preventDefault();
            }
        });
        ...
        document.addEventListener('touchend', (e) => {
            ...
            // Umbral más alto (100px en lugar de 50px) para menor sensibilidad
            const threshold = 100;
            
            // Solo cambiar página si el movimiento es PRINCIPALMENTE horizontal
            // y supera el umbral
            if (Math.abs(diffX) > threshold && Math.abs(diffX) > diffY * 2) {
                if (diffX > 0) {
                    // Swipe izquierda - página siguiente
                    console.log('Cambio a página siguiente');
                    this.nextPage();
                } else {
                    // Swipe derecha - página anterior
                    console.log('Cambio a página anterior');
                    this.prevPage();
                }
            }
